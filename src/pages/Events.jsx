...
const EventCard = ({ event, facebookLink, formatDate, isPastEvent = false }) => {
  ...
  return (
    <div className={`bg-white rounded-lg overflow-hidden shadow-md ${isPastEvent ? 'opacity-75' : ''}`}>
      ...
      <div className="p-6">
        <div className={`text-sm mb-2 ${isPastEvent ? 'text-gray-500' : 'text-gray-600'}`}>
          {!event.date ? "Date and time to be announced" : formatDate(event)}
        </div>
        <h3 className={`text-xl font-semibold mb-2 ${isPastEvent ? 'text-gray-600' : 'text-gray-900'}`}>
          {event.title}
        </h3>
        <p className={`mb-4 ${isPastEvent ? 'text-gray-500' : 'text-gray-600'}`}>
          {event.description}
        </p>
        <div className={`flex items-center mb-4 ${isPastEvent ? 'text-gray-500' : 'text-gray-700'}`}>
        ...
        <a
          ...
          className={`font-medium ${isPastEvent ? 'text-gray-500 hover:text-gray-700' : 'text-gray-700 hover:text-black'}`}
        >
          Learn More →
        </a>
      </div>
...
const EventList = ({ events, isPastEvents = false }) => {
  ...
  const groupedEvents = React.useMemo(() => {
    ...
    const recurring = events.filter(event => {
      // Check if this is a recurring event by looking at the ID pattern
      const recurringPatterns = ['sunday-service', 'kids-church', 'encounter-night', 'community-service', 'water-baptism'];
      return recurringPatterns.some(pattern => event.id.includes(pattern));
    });
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {groupedEvents.recurring.map(event => (
              <EventCard key={event.id} event={event} facebookLink={facebookLink} formatDate={formatDate} isPastEvent={isPastEvents} />
            ))}
          </div>
        </>
      )}
  ...
  return (
    <div>
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {/* Future Events */}
        <EventList events={futureEvents} isPastEvents={false} />
        
        {/* Past Events */}
        {pastEvents.length > 0 && (
          <>
            <h3 className="text-2xl font-semibold mb-6 mt-16">Past Events</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {pastEvents.map(event => (
                <EventCard key={event.id} event={event} facebookLink="https://www.facebook.com/HopeCanberra" formatDate={formatEventDate} isPastEvent={true} />
              ))}
            </div>
          </>
        )}
